  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
        ...
        <img src={assets.help_icon} alt="info icon" className='max-md:hidden max-w-5' />
      </div>
      {/* chat area */}
        ...
        {messagesDummyData.map((msg,index)=>(
            <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== '680f50e4f10f3cd28382ecf9' && 'flex-row-reverse'}`}>
                ...
                <div className='text-center text-xs'>
                    <img src={msg.senderId === '680f50e4f10f3cd28382ecf9' ? assets.avatar_icon : assets.profile_martin} alt="image" className='w-7 rounded-full' />
      ...
      <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
        <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
            <input type="text" placeholder='Send Message' className='flex-1 text-sm p-3 border-none outline-none rounded-lg text-white placeholder-gray-400' />
            <input type="file" id='image' accept='image/png, image/jpg, image/jpeg' hidden />
            <label htmlFor="image">
                <img src={assets.gallery_icon} alt="image icon" className='w-5 mr-2 cursor-pointer' />
            </label>
        </div>
        <img src={assets.send_button} alt="send icon" className='w-7 cursor-pointer' />
      </div>
    </div>
  ) : (
